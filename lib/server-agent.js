var tls = require('tls');
var Q = require('q');
var getServer = require('./get-server').create(createTLSServer, 41500);
...
proto.existsServer = function(hostname) {
  var promise = this._cache[hostname];
  return promise && promise.server;
};
proto.createServer = function createServer(hostname, listener, callback) {
  ...
  self.freeServer();
  getServer(self.createCertificate(hostname), listener, function(server, port) {
